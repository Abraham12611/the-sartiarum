    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-6" style={{ background: '#F9F6EE', fontFamily: 'Inter, sans-serif' }}>
      <div style={{ width: '100%', maxWidth: 380, background: '#fff', borderRadius: 20, boxShadow: '0 8px 48px rgba(0,0,0,0.10)', padding: '36px 32px' }}>
        <div className="flex justify-center mb-5">
          <Image src="/logo.png" alt="Sartiarum" width={30} height={30} style={{ objectFit: 'contain' }} />
        </div>

        {sent ? (
          <>
            <h1 style={{ fontSize: 22, fontWeight: 800, color: '#141516', textAlign: 'center', marginBottom: 12 }}>Check your inbox</h1>
            <p style={{ fontSize: 14, color: '#4F5963', textAlign: 'center', lineHeight: 1.6 }}>
              We sent a reset link to <strong>{email}</strong>. Check your email and follow the link to reset your password.
            </p>
            <Link
              href="/login"
              className="flex items-center justify-center gap-2"
              style={{ marginTop: 24, fontSize: 14, fontWeight: 600, color: '#4F6F3D', textDecoration: 'none' }}
            >
          ...
          <>
            ...
            <form onSubmit={handleSubmit}>
              {error && (
                <div style={{ fontSize: 13, color: '#dc2626', background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 10, padding: '10px 14px', marginBottom: 14 }}>
                  {error}
                </div>
              )}
              ...
              <button
                type="submit"
                disabled={loading}
                style={{ width: '100%', height: 44, borderRadius: 12, background: '#4F6F3D', color: '#fff', fontSize: 15, fontWeight: 600, border: 'none', cursor: 'pointer', boxShadow: '0 4px 14px rgba(79,111,61,0.25)', fontFamily: 'Inter, sans-serif' }}
              >
            ...
            <Link
              href="/login"
              className="flex items-center justify-center gap-2"
              style={{ marginTop: 20, fontSize: 14, color: '#4F5963', textDecoration: 'none' }}
            >
              <ArrowLeft size={15} /> Back to login
            </Link>
          </>
