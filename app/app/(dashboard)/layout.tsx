...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const [spacesData, boardsData, profileData, subData, usageCount] = await Promise.all([
    ...
    db.select({ value: dbCount() }).from(aiUsage).where(and(eq(aiUsage.ownerId, user.id), gte(aiUsage.createdAt, thirtyDaysAgo))),
  ])

  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: '#fff', fontFamily: 'Inter, sans-serif' }}>
      <Sidebar spaces={spacesData} boards={boardsData} profile={profileData[0] ?? null} subscription={subData[0] ?? null} aiUsageCount={usageCount[0]?.value ?? 0} userId={user.id} />
