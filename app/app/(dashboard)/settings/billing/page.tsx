    ...
    <div style={{ padding: '40px 48px', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ maxWidth: 480, background: '#fff', border: '1px solid #e4ddd5', borderRadius: 14, padding: '28px 28px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <div>
            <p style={{ fontSize: 15, fontWeight: 700, color: '#141516', marginBottom: 2 }}>
              {isTrialing ? 'Free Trial' : sub?.status === 'active' ? 'Pro' : 'Inactive'}
            </p>
            {isTrialing && trialEnds && (
              <p style={{ fontSize: 13, color: '#4F5963' }}>
                Expires {formatDistanceToNow(trialEnds, { addSuffix: true })}
              </p>
            )}
          </div>
          <span style={{ fontSize: 12, fontWeight: 600, padding: '4px 10px', borderRadius: 20, background: isTrialing ? '#eef2e9' : '#f0f0f0', color: isTrialing ? '#4F6F3D' : '#4F5963' }}>
            {sub?.status ?? 'unknown'}
          </span>
        </div>
