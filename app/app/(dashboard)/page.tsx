...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<{ board?: string }> }) {
  ...
  if (!activeBoardId) {
    const [pinned] = await db.select().from(boards).where(and(eq(boards.ownerId, user.id), eq(boards.isPinned, true))).orderBy(boards.sortOrder).limit(1)
  ...
  if (!activeBoardId) return <div style={{ display: 'flex', flex: 1, alignItems: 'center', justifyContent: 'center', height: '100%', color: '#9AA4A0', fontSize: 15 }}>Setting up your workspace…</div>
  ...
  return (
    <div style={{ display: 'flex', height: '100%' }}>
      <BoardView board={activeBoard[0]} sections={boardSections} documents={boardDocs} userId={user.id} />
      <StatsPanel totalDocuments={totalCount[0]?.value ?? 0} recentDocuments={recentDocs} />
    </div>
  )
}
