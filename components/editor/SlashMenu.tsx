  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Enter') {
        ...
        if (cmd) {
          // Delete the slash + query from the editor
  ...
  useEffect(() => {
    ...
    const unsubscribe = editor.on('update', () => {
      ...
      if (slashMatch) {
        ...
        const editorEl = editor.view.dom.closest('.tiptap-editor') ?? editor.view.dom
      ...
      {filtered.map((cmd, i) => (
        <button
          ...
          style={{
            ...
            padding: '9px 14px',
            border: 'none',
            background: i === selectedIdx ? '#f0ede8' : 'transparent',
            cursor: 'pointer',
            textAlign: 'left',
        ...
        >
          ...
          <div>
            <p style={{ fontSize: 13.5, fontWeight: 600, color: '#141516', margin: 0 }}>{cmd.label}</p>
            <p style={{ fontSize: 11.5, color: '#9AA4A0', margin: 0 }}>{cmd.description}</p>
          </div>
        </button>
      ))}
    </div>
